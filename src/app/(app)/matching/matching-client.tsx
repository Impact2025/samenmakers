...
export function MatchingClient() {
  ...
  const [matchResult, setMatchResult] = useState<{ naam: string; avatarUrl: string | null | undefined } | null>(null);
  ...
  const swipe = trpc.matches.swipe.useMutation({
    onSuccess: (data) => {
      ...
      if (data.matched && data.target) {
        setMatchResult({ naam: data.target.naam ?? "een maker", avatarUrl: data.target.avatarUrl });
      }
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-6">
          <div className="bg-white max-w-sm w-full p-8 text-center">
            ...
            <p className="text-body text-on-surface-variant mb-6">
              Jullie zijn beiden geïnteresseerd. Stuur een bericht om te beginnen.
            </p>
        ...
        <div className="space-y-4">
              ...
              <div className="flex gap-5 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {current.sector && (
                      <Badge variant="default" size="sm">{current.sector}</Badge>
                    )}
                    {current.regio && (
                      <Badge variant="default" size="sm">{current.regio}</Badge>
                    )}
                    {current.fase && (
          ...
          <div className="flex items-center justify-center gap-6">
            <button
              ...
              className="w-16 h-16 border border-hairline flex items-center justify-center text-outline hover:border-red-400 hover:text-red-400 transition-colors disabled:opacity-50"
              aria-label="Sla over"
            >
              <X size={24} />
            </button>
            ...
            <button
              ...
              disabled={swipe.isPending}
              className="w-20 h-20 bg-primary flex items-center justify-center text-on-primary hover:bg-primary-container transition-colors disabled:opacity-50"
              aria-label="Connect"
            >
      ...
      {myMatches && myMatches.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/berichten" className="text-label-caps text-primary">
              BERICHTEN →
            </Link>
          </div>
